...
void Lvgl_Init(void)
{
  ...
  //lv_style_set_border_width(&border_style, 5);              // 设置边框宽度
  //lv_style_set_border_color(&border_style, lv_color_hex(0xFF0000)); // 设置边框颜色为红色
  //lv_style_set_border_opa(&border_style, LV_OPA_COVER);     // 设置边框不透明度
  //lv_obj_t *screen = lv_scr_act(); // 获取当前屏幕对象
  //lv_obj_add_style(screen, &border_style, 0); // 将边框样式应用到屏幕?
