...
void LCD_Init(void)
{
  ...
  // 绘制绿色边框，宽度为5像素
  //LCD_DrawBorder(0, 0, LCD_WIDTH - 1, LCD_HEIGHT - 1, 0xF800, 5);
}
/******************************************************************************
...
void LCD_DrawBorder(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color, uint8_t thickness) {
    // 绘制上边框
    LCD_SetCursor(x1, y1, x2, y1 + thickness - 1);
    ...
    // 绘制下边框
    LCD_SetCursor(x1, y2 - thickness + 1, x2, y2);
    for (uint16_t i = x1; i <= x2; i++) {
        for (uint8_t j = 0; j < thickness; j++) {
            LCD_WriteData_Word(color);
        }
    ...
    // 绘制左边框
    LCD_SetCursor(x1, y1, x1 + thickness - 1, y2);
    ...
    // 绘制右边框
    LCD_SetCursor(x2 - thickness + 1, y1, x2, y2);
    for (uint16_t i = y1; i <= y2; i++) {
        for (uint8_t j = 0; j < thickness; j++) {
            LCD_WriteData_Word(color);
        }
